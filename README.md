...
2. 下記のディレクトリで画面設計書、DB項目定義書、単体テスト仕様書などをマークダウン形式で一から作成し、**実務の中で経験した詳細設計から結合テストまでの工程を一人称で進める**ことでより現場を想定した実装ができるようにしました。(単体テスト内でDBとの接続のような結合テストにあたるテストを実施しているので結合テストとしてのドキュメント作成や実施は省略しています)
